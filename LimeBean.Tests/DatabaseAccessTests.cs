    ...
    public class DatabaseAccessTests {
        ...
        public void Caching_EnableDisable() {
            var readers = new Func<string, IConvertible>[] { 
                sql => _db.Cell<string>(sql),
                sql => _db.Col<string>(sql)[0],
                sql => _db.Row(sql)["r"],
                sql => _db.Rows(sql)[0]["r"],
            };
        ...
        public void Caching_InvalidateByUpdate() {            
            ...
            _db.Cell<int>("select a from t1");
            _db.Exec("update t1 set a=82");
            Assert.AreEqual(82, _db.Cell<int>("select a from t1"));
        }

        [Test]
        public void UncachedRead() { 
            var uuid = _db.Cell<string>(SELECT_RANDOM);
            Assert.AreEqual(uuid, _db.Cell<string>(SELECT_RANDOM));
            Assert.AreNotEqual(uuid, _db.Cell<string>(false, SELECT_RANDOM));

            Assert.AreNotEqual(uuid, _db.Cell<string>(SELECT_RANDOM), "No stale cache");
        }
        ...
        public void Transactions() {
            ...
            Assert.AreEqual(0, _db.Cell<int>("select count(*) from t"));
            ...
            Assert.AreEqual(1, _db.Cell<int>("select count(*) from t"));
        }
        ...
        public void RolledBackTransactionClearsCache() {
            ...
            _db.Transaction(delegate() {
                ...
                _db.Cell<int>("select x from foo");
                return false;
            });

            Assert.AreEqual(1, _db.Cell<int>("select x from foo"));
        }
    }
}
